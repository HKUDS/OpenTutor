...
test.describe('Compliance :: Error Handling & UX Signals', () => {
  test('api error surfaces user-friendly feedback (alert or message)', async ({ page }) => {
    ...
    const roleAlert = page.getByRole('alert').first()
    const deterministicError = page.locator('[data-test=error-message]').first()
    const safeFallback = page.locator('[data-test=notebooks-empty]').first()

    await expect(roleAlert.or(deterministicError).or(safeFallback)).toBeVisible({ timeout: 5000 })
  })
})
